...
- 面向对象设计原则
  ...
  - 单一职责原则 （**S**RP）- 一个类或接口的职责明确且唯一，即“高内聚，低耦合”；
  - 开闭原则 （**O**CP）- 对扩展开发，对修改关闭，即当你需要修改一个类时，你可以继承它，添加接口，而不是修改接口，添加代码，而不是修改代码；
  - 依赖倒转原则（DIP）- 面向抽象编程（在弱类型语言中已经被弱化）依赖于抽象而不依赖于具体，我们尽量使用接口来规范依赖；
  - 里氏替换原则（**L**SP） - 任何时候可以用子类对象替换掉父类对象，父类可以出现的地方，子类都可以出现
  - 接口隔离原则（**I**SP）- 接口要小而专不要大而全（Python中没有接口的概念，每一个接口的职责单一明确，所以当我们要实现一个复杂的类时，我们尽量去实现多个接口，
  - 合成聚合复用原则（CARP） - 优先使用强关联关系而不是继承关系复用代码，尽量使用对象组合，而不是继承来达到复用的目的。
  - 最少知识原则（迪米特法则，Lo**D**）- 不要给没有必然联系的对象发消息，对象之间应该要尽量少的了解对方；
